...
import { uploadFilesAsAttachments } from './attachments';
...
const sendMessage = (
  ...
  files?: MessageConfig['files']
) => new Promise<string>((resolve) => {
  let textValue = '';
  const textDelta$ = sendMessageReceiveDelta(threadId, message, files);
...
const sendMessageReceiveDelta = (
  ...
  const queueEntry = [{ message, files }, onTextDelta$] as QueueEntry;
...
const runMessageQueue = async (threadId: string) => {
  ...
  const [{ message, files }, onTextDelta$] = MessageQueue[threadId].shift()!;

  const createMessageAndStream = async () => {
    const handleAPIError = (restartRequest = false) => {
      if (restartRequest) {
        MessageQueue[threadId].unshift([{ message, files }, onTextDelta$]);
